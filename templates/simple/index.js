...
const DOCTYPE_MAP = {
  "Teklif": "offer", "Proforma": "proforma", "Fatura": "invoice",
  "Sipariş": "order", "Sözleşme": "contract",
};

function buildTerms(offerTerms, docType) {
  if (!Array.isArray(offerTerms) || offerTerms.length === 0) return [];
  const key = DOCTYPE_MAP[docType] || (docType || "").toLowerCase();
  const visible = offerTerms.filter((t) => {
    if (!t.isVisible) return false;
    if (Array.isArray(t.visibleIn) && t.visibleIn.length > 0 && !t.visibleIn.includes(key)) return false;
    const val = t.value;
  ...
  const rows = visible.map((t) => [
    ...
    { text: Array.isArray(t.value) ? t.value.join(", ") : String(t.value), fontSize: 8.5, color: C.body, lineHeight: 1.3 },
  ]);
  return [
    { canvas: [{ type: "line", x1: 0, y1: 0, x2: 523, y2: 0, lineWidth: 0.4, lineColor: C.border }], margin: [0, 4, 0, 4] },
    sectionLabel("SATIŞ KOŞULLARI"),
    {
      table: { widths: [110, "*"], body: rows },
      layout: {
        hLineWidth: (i, n) => (i === 0 || i === n.table.body.length) ? 0.6 : 0.4,
        vLineWidth: () => 0, hLineColor: () => C.border,
        fillColor: (i) => i % 2 === 0 ? C.bg : null,
...
function buildBankAccounts(accounts) {
  if (!Array.isArray(accounts) || accounts.length === 0) return [];

  const rows = accounts.map((acc) => [
    { text: `${s(acc.bankName)}`, style: "fieldVal", bold: true },
    { text: `${s(acc.accountNumber)}`, style: "fieldVal" },
    { text: `${s(acc.currency)}`, style: "fieldVal" },
  ]);
  ...
  return [
    ...
    sectionLabel("BANKA HESAPLARI"),
    {
      table: {
        ...
        body: [
          [
            { text: "BANKA", style: "tableHeader", fillColor: "#0ea5e9" },
            { text: "IBAN / HESAP NO", style: "tableHeader", fillColor: "#0ea5e9" },
            { text: "PARA BİRİMİ", style: "tableHeader", fillColor: "#0ea5e9", alignment: "center" },
          ],
          ...rows,
        ],
      },
      layout: {
        ...
        vLineWidth: () => 0,
        hLineColor: () => "#e5e7eb",
        fillColor: (i) => (i === 0 ? "#0ea5e9" : i % 2 === 0 ? "#f9fafb" : null),
        paddingTop: () => 4,
        paddingBottom: () => 4,
        paddingLeft: () => 4,
        paddingRight: () => 4,
